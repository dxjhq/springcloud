...
import com.hhly.token.encrypt.jwt.JWTExpiredException;
import com.hhly.token.encrypt.jwt.JWTSigner;
import com.hhly.token.encrypt.jwt.JWTVerifier;
import com.hhly.token.encrypt.jwt.JWTVerifyException;
...
public final class Encrypt {
    ...
    public static String aesEncode(String data) {
        ...
        if (AES_SECRET.length != 16) return "密钥必须是16位";
        try {
    ...
    public static Map<String, Object> jwtDecode(String data) {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("使用 jwt 解密(" + data + ")时, 数据已过期", e);
            }
        }
//        catch (NoSuchAlgorithmException | InvalidKeyException | IOException |
//                SignatureException | JWTVerifyException e) {
//            if (logger.isDebugEnabled()) {
//                logger.debug("使用 jwt 解密(" + data + ")失败", e);
//            }
//        }
        catch (Exception e) {
            if (logger.isDebugEnabled()){
                logger.debug("使用 jwt 解密(" + data + ")异常", e);
            }
        }
