...
import com.hhly.api.enums.DateFormatType;
...
public final class ExportUtil {
    ...
    public static enum Type {
        ...
        public boolean isExcel() {return Arrays.asList(MS03, MS07).contains(this);}
    ...
    public static void export(Type type, String name, LinkedHashMap<String, String> titleMap,
        ...
        if (type.isExcel()) {
            exportXls(response, fileName, titleMap, AssembleUtil.linkedMaps(name, dataList));
    ...
    public static String encodeName(String name) {
        String fileName = name + "-" + (com.hhly.utils.date.DateUtil.formatDateToString(new java.util.Date(), DateFormatType.YYYYMMDD.getValue()));
        String userAgent = RequestUtil.userAgent();
        if (ValueUtil.isNotBlank(userAgent) && userAgent.contains("Mozilla")) {
    ...
    public static void exportXls(HttpServletResponse response, String fileName,
        ...
        ExportExcel.handle(Type.is07Suffix(fileName), titleMap, dataMap).write(response.getOutputStream());
    }
    ...
    private static String convertCsv(LinkedHashMap<String, String> titleMap, List<?> dataList) {
        ...
        if (AssembleUtil.isEmpty(titleMap) || AssembleUtil.isEmpty(dataList)) return ValueUtil.EMPTY;
        ...
        for (Object data : dataList) {
            ...
            for (String title : titleMap.keySet()) {
                sbd.append(handleCsvContent(ValueUtil.getField(data, title)));
                i++;
                if (i != titleMap.size()) sbd.append(",");
            }
        }
    ...
    private static final class ExportExcel {
        ...
        private static Workbook handle(boolean excel07, LinkedHashMap<String, String> titleMap,
            ...
            // 没有数据, 或者没有标题, 都直接返回
            if (AssembleUtil.isEmpty(dataMap) || AssembleUtil.isEmpty(titleMap)) return workbook;
                ...
                if (AssembleUtil.isNotEmpty(excelList)) {
                    ...
                    for (int i = 0; i < sheetCount; i++) {
                        ...
                        for (String header : titleMap.values()) {
                            ...
                            cell.setCellValue(ValueUtil.getNil(header));
                            cellIndex++;
                        }
                        // 冻结第一行
                        ...
                        for (Object data : excelList.subList(fromIndex, toIndex)) {
                            if (data != null) {
                                ...
                                for (String value : titleMap.keySet()) {
                                    ...
                                    cell.setCellValue(ValueUtil.getField(data, value));
