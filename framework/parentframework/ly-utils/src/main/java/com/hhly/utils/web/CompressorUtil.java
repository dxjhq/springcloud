...
public final class CompressorUtil {
    private static final String CLAZZ_NAME = CompressorUtil.class.getName();
    ...
    public static String html(String html) {
        if (ValueUtil.isBlank(html)) return ValueUtil.EMPTY;

        // 收集 <script> 标签. 这个特殊的标签里面的内容要单独处理
        List<String> scriptList = AssembleUtil.linkedLists();

        // 收集 if 标签: <!--[if lt IE 7 ]> <html class="ie ie6"> <![endif]-->
        List<String> ifList = AssembleUtil.linkedLists();
        // 收集 <pre> <textarea> 标签, 这些特殊标签里面的内容不需要压缩, 压缩将会导致内容显示错误
        List<String> preList = AssembleUtil.linkedLists();
        List<String> textareaList = AssembleUtil.linkedLists();
    ...
    private static String receiveJs(String html, List<String> ignoreList) {
        if (ValueUtil.isNotBlank(html) && AssembleUtil.isNotEmpty(ignoreList)) {
            String place;
            while (html.contains(SCRIPT_PLACE)) {
                place = ignoreList.remove(0);
                if (ValueUtil.isNotBlank(place)) {
                    List<String> scriptStrList = AssembleUtil.linkedLists();
                    // 收集 js 代码中的字符串
